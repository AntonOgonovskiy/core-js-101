...
function findFirstSingleChar(str) {
  for (let j = 0; j < str.length; j += 1) {
    if (!(str.slice(0, j)).includes(str[j]) && !(str.slice(j + 1)).includes(str[j])) {
      return str[j];
    }
  }
  return null;
...
function getDigitalRoot(num) {
  ...
  return nums.toString().length > 1 ? getDigitalRoot(nums) : nums;
}
...
function toNaryString(num, n) {
  let sum = '';
  for (let i = num; i >= 1; i = Math.floor(i / n)) {
    sum += i % n;
  }
...
function getCommonDirectoryPath(pathes) {
  const absPath = pathes.map((item) => item.split('/'));
  const path = pathes.map((item) => item.split('/'))
    .flat()
    .filter((item, index, arr) => arr.includes(item, index + 1))
    .filter((item) => absPath.every((str) => str.includes(item)))
    .join('/');
  if (path.length > 1) {
    return `${path}/`;
  }
...
function evaluateTicTacToePosition(matrix) {
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    ...
    if (arr1[0] === arr1[1] && arr1[0] === arr1[2] && arr1[0] !== undefined) {
      winner += arr1[0];
    }
    arr1 = [];
  }
  ...
  for (let i = 0; i < matrix.length; i += 1) {
    arr1.push(matrix[i][matrix[i].length - 1 - i]);
  }
  if (arr1[0] === arr1[1] && arr1[0] === arr1[2] && arr1[0] !== undefined) {
    winner += arr1[0];
  }
